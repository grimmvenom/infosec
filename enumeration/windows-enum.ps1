...
if ($result -eq $False) {
    Write-Host -ForegroundColor Red  "C:\ Temp does not Exist"
    Write-Host -ForegroundColor Green  "Creating C:\temp"
    New-Item C:\temp -type directory
}

Start-Transcript -Path "C:\temp\powershell-enum.txt"
RunCommands($standard_commands)

if ($extended) {
    if ($extended.ToLower() -eq 'extended') {
        whost "Running Extended Commands.... This may take a while"
